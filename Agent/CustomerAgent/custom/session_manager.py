    ...
    def _init_encoding(self) -> None:
        try:
            ...
            try:
                ...
                logger.warning(f"tiktoken 编码器初始化失败: {_sanitize_for_log(e)}")
                self._available = False
...
class SessionManager:
    ...
    def __init__(
        ...
        self.engine = engine or self._shared_engine(db_path)
    ...
    @staticmethod
    def _shared_engine(db_path: str) -> Engine:
        try:
            from database import get_db_manager

            manager = get_db_manager()
            manager_db_path = os.path.abspath(os.fspath(getattr(manager, "db_path", "")))
            requested_db_path = os.path.abspath(os.fspath(db_path))
            if manager_db_path == requested_db_path:
                return manager.engine
        except Exception as exc:
            logger.debug(f"复用全局数据库连接失败，使用会话库独立连接: {_sanitize_for_log(exc)}")
        return create_engine(f"sqlite:///{db_path}", connect_args={"timeout": 30})
    ...
    def add_message(
        ...
        try:
            ...
            session.rollback()
            logger.error(f"添加消息失败: {_sanitize_for_log(e)}")
            return False
        finally:
            session.close()
    ...
    def get_history(
        ...
        try:
            ...
            logger.error(f"获取历史消息失败: {_sanitize_for_log(e)}")
            return []
        finally:
            session.close()
    ...
    def get_message_count(self, session_id: str) -> int:
        ...
        try:
            ...
            logger.error(f"获取消息数量失败: {_sanitize_for_log(e)}")
            return 0
        finally:
            session.close()
    ...
    def compress_history(
        ...
        try:
            ...
            try:
                summary = str(llm_callable(summary_input) or "").strip()
            except Exception as e:
                logger.error(f"生成摘要失败，保留原历史不压缩: {_sanitize_for_log(e)}")
                return False
            if not summary:
                logger.error("生成摘要为空，保留原历史不压缩")
                return False
            ...
            session.rollback()
            logger.error(f"压缩历史失败: {_sanitize_for_log(e)}")
            return False
        finally:
            session.close()
