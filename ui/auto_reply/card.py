...
from qfluentwidgets import CardWidget, StrongBodyLabel, CaptionLabel, BodyLabel, PushButton, PrimaryPushButton, InfoBadge, FluentIcon as FIF, ComboBox
...
class AutoReplyCard(CardWidget):
    ...
    transfer_target_changed = pyqtSignal(dict, object, str)
    ...
    def createActionWidget(self):
        ...
        transfer_label = CaptionLabel("转人工:")
        transfer_label.setStyleSheet("color: #7f8c8d; font-weight: 500;")
        transfer_label.setFixedWidth(60)

        self.transfer_combo = ComboBox()
        self.transfer_combo.setMinimumWidth(220)

        transfer_layout.addWidget(transfer_label)
        transfer_layout.addWidget(self.transfer_combo, 1)

        action_layout.addWidget(status_badge, 0, Qt.AlignmentFlag.AlignRight)
        action_layout.addWidget(buttons_widget)
        action_layout.addWidget(transfer_widget)
    ...
    def connectSignals(self):
        ...
        self.transfer_combo.currentIndexChanged.connect(self._onTransferTargetChanged)
    ...
    def setTransferOptions(self, options: list, selected_cs_uid=None):
        """设置转人工账号选项。"""
        self.transfer_combo.blockSignals(True)
        self.transfer_combo.clear()

        self.transfer_combo.addItem("自动选择", userData={"cs_uid": None, "username": ""})

        selected_index = 0
        for index, item in enumerate(options, start=1):
            username = item.get("username", "")
            status_text = item.get("status_text", "")
            label = username if not status_text else f"{username}（{status_text}）"
            self.transfer_combo.addItem(
                label,
                userData={
                    "cs_uid": item.get("cs_uid"),
                    "username": username,
                },
            )
            if selected_cs_uid and str(item.get("cs_uid")) == str(selected_cs_uid):
                selected_index = index

        self.transfer_combo.setCurrentIndex(selected_index)
        self.transfer_combo.blockSignals(False)

    def _onTransferTargetChanged(self, index: int):
        """转人工目标变更回调。"""
        data = self.transfer_combo.itemData(index)
        if not isinstance(data, dict):
            data = {"cs_uid": None, "username": ""}
        self.transfer_target_changed.emit(
            self.account_data,
            data.get("cs_uid"),
            data.get("username", ""),
        )
