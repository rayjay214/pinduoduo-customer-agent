...
from utils.transfer_target import build_transfer_candidates, normalize_cs_uid
...
class AutoReplyUI(QFrame):
    ...
    def refreshAccountList(self):
        ...
        for account_data in self.accounts_data:
            ...
            account_card.transfer_target_changed.connect(self.onTransferTargetChanged)
            ...
            self._apply_transfer_target_options(account_card)
    ...
    @staticmethod
    def _status_text(status_value) -> str:
        status_map = {
            0: "休息",
            1: "在线",
            3: "离线",
    ...
    def _get_transfer_candidates_for_account(self, account_data: dict) -> list:
        """优先从平台后台获取当前在线客服列表。"""
        online_candidates = []
        ...
        try:
            ...
            sender = SendMessage(shop_id, user_id, channel_name)
            cs_list = sender.getAssignCsList()
            if cs_list and isinstance(cs_list, dict):
                for candidate in build_transfer_candidates(shop_id, user_id, cs_list):
                    online_candidates.append(
                        {
                            "cs_uid": candidate["cs_uid"],
                            "username": candidate["username"],
                            "status_text": "在线",
                        }
                    )
        except Exception as e:
            self.logger.warning(f"获取平台在线客服列表失败，回退本地账号: {e}")

        if not online_candidates:
            same_shop_accounts = [
                item for item in self.accounts_data
                if item.get("channel_name") == channel_name
                and str(item.get("shop_id")) == shop_id
                and str(item.get("user_id")) != user_id
                and item.get("status") == 1
            ]
            for item in same_shop_accounts:
                online_candidates.append(
                    {
                        "cs_uid": f"cs_{shop_id}_{item.get('user_id')}",
                        "username": item.get("username"),
                        "status_text": "本地在线",
                    }
                )

        configured = db_manager.get_transfer_target(
            channel_name,
            shop_id,
            user_id,
        )
        configured_cs_uid = (
            normalize_cs_uid(shop_id, configured.get("target_user_id"))
            if configured else None
        )

        if configured and configured_cs_uid and not any(
            str(item.get("cs_uid")) == str(configured_cs_uid) for item in online_candidates
        ):
            online_candidates.append(
                {
                    "cs_uid": configured_cs_uid,
                    "username": configured.get("target_username") or str(configured_cs_uid),
                    "status_text": "当前不可选",
                }
            )
    ...
    def _apply_transfer_target_options(self, account_card: AutoReplyCard):
        """将转人工账号选项应用到卡片。"""
        account_data = account_card.account_data
        configured = db_manager.get_transfer_target(
            account_data.get("channel_name", "pinduoduo"),
            str(account_data.get("shop_id", "")),
            str(account_data.get("user_id", "")),
        )
        selected_cs_uid = (
            normalize_cs_uid(str(account_data.get("shop_id", "")), configured.get("target_user_id"))
            if configured else None
        )
        options = self._get_transfer_candidates_for_account(account_data)
        account_card.setTransferOptions(options, selected_cs_uid)
            ...
            for i in range(self.accounts_layout.count() - 1):
                ...
                if isinstance(widget, AutoReplyCard):
                    ...
                    if is_running != current_status:
                        ...
                        self._apply_transfer_target_options(widget)
                        updated_count += 1
    ...
    def onTransferTargetChanged(self, account_data: dict, target_cs_uid, target_username: str):
        """保存转人工目标配置。"""
        try:
            shop_id = str(account_data.get("shop_id", ""))
            normalized_target_cs_uid = normalize_cs_uid(shop_id, target_cs_uid) if target_cs_uid else None
            success = db_manager.set_transfer_target(
                account_data.get("channel_name", "pinduoduo"),
                shop_id,
                str(account_data.get("user_id", "")),
                normalized_target_cs_uid,
                target_username,
            )
            if success:
                if normalized_target_cs_uid:
                    self.logger.info(
                        f"账号 '{account_data['username']}' 已设置优先转人工到 '{target_username}' ({normalized_target_cs_uid})"
                    )
                else:
                    self.logger.info(f"账号 '{account_data['username']}' 已清除优先转人工账号，恢复自动选择")
        except Exception as e:
            self.logger.error(f"保存转人工目标配置失败: {e}")
    ...
    def onStatusSetSuccess(self, account_data: dict, new_status: int):
        ...
        try:
            ...
            self._refresh_transfer_options_for_all_cards()
    ...
    def _refresh_transfer_options_for_all_cards(self):
        """刷新所有卡片的转人工账号选项。"""
        for i in range(self.accounts_layout.count() - 1):
            widget = self.accounts_layout.itemAt(i).widget()
            if isinstance(widget, AutoReplyCard):
