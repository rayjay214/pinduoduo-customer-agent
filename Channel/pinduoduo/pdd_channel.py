...
class PDDChannel(ConnectionMixin, MessageHandlerMixin, LifecycleMixin, StatusMixin, Channel):
    ...
    def __init__(self, max_concurrent_messages: int = 50, status_manager: ConnectionStatusManager = None):
        ...
        self._stop_event: Optional[asyncio.Event] = None  # 兼容旧调用；新逻辑按账号使用 _stop_events
        self._stop_events: Dict[str, asyncio.Event] = {}
        self.base_url = "wss://m-ws.pinduoduo.com/"
        self.ws: Optional[websockets.WebSocketClientProtocol] = None  # 兼容旧调用；新逻辑按账号使用 _websockets
        ...
        self._processing_tasks_by_connection: Dict[str, Set[asyncio.Task[Any]]] = {}
