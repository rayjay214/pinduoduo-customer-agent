...
class SimpleLoggerConfig:
    ...
    def is_business_logging_enabled(self) -> bool:
        """检查是否启用业务日志"""
        return as_bool(os.environ.get("BUSINESS_LOGGING", "true"), True)

    def is_ui_logging_enabled(self) -> bool:
        """检查是否启用UI日志"""
        return as_bool(os.environ.get("UI_LOGGING", "true"), True)

    def get_log_retention_days(self) -> int:
        """获取日志保留天数"""
        return max(1, as_int(os.environ.get("LOG_RETENTION_DAYS", "7"), 7))
