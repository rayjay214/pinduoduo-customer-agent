...
class WebSocketResourceManager:
    ...
    def register_websocket(self, websocket: Any, name: str = None) -> None:
        ...
        def cleanup_callback(ref):
            ...
            self._schedule_reference_cleanup(ref)
    ...
    async def cleanup_all(self) -> None:
        ...
        async with self._lock:
            ...
            for ref in connections_copy:
                ...
                if websocket is not None:
                    try:
                        ...
                        error_msg = f"清理WebSocket失败: {safe_error}"
            ...
            if errors:
                ...
                for error in errors:
                    self.logger.error(f"  - 清理WebSocket失败: {_sanitize_for_log(error)}")
            else:
    ...
    def _schedule_reference_cleanup(self, ref: weakref.ref) -> None:
        try:
            loop = asyncio.get_running_loop()
        except RuntimeError:
            self._connections.discard(ref)
            self.logger.debug("WebSocket连接已被垃圾回收，未检测到运行中的事件循环，直接清理引用")
            return
        loop.create_task(self._cleanup_reference(ref))
