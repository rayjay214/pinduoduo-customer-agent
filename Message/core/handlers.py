...
class MessageHandler(ABC):
    ...
    async def on_error(self, context: Context, error: Exception) -> None:
        ...
        if isinstance(error, BaseException):
            text = str(error)
            safe_error = f"{type(error).__name__}: {BaseRequest()._sanitize_for_log(text)}" if text else type(error).__name__
        else:
            safe_error = BaseRequest()._sanitize_for_log(error)
        self.logger.error(f"Handler {self.__class__.__name__} error: {safe_error}")
...
class CatchAllHandler(MessageHandler):
    ...
    async def handle(self, context: Context, metadata: Dict[str, Any]) -> bool:
        ...
        self.logger.info(f"消息内容长度: content_chars={content_chars}")
        self.logger.info(f"消息未被业务处理器确认处理")
        self.logger.info(f"===================")
