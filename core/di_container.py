...
class DIContainer:
    ...
    def _create_instance(self, implementation_type: Type) -> Any:
        ...
        try:
            ...
            self.logger.error(f"创建实例失败 {implementation_type.__name__}: {_sanitize_for_log(e)}")
    ...
    async def _create_instance_async(self, implementation_type: Type) -> Any:
        ...
        try:
            ...
            self.logger.error(f"异步创建实例失败 {implementation_type.__name__}: {_sanitize_for_log(e)}")
            raise
            ...
            for instance in self._singletons.values():
                if hasattr(instance, 'dispose') and callable(getattr(instance, 'dispose')):
                    try:
                        ...
                        self.logger.error(f"释放实例失败: {_sanitize_for_log(e)}")
